...
public class ClientSkinCache
{
	...
	public void setLoaded(String name, int w, int h)
	{
		...
		Identifier id = CustomClientSkinManager.idFromName(name);
	...
	public void clear()
	{
		...
		for (Map.Entry<String, ClientAsset.ResourceTexture> entry : textureByName.entrySet())
		{
			...
			{
				Identifier id = CustomClientSkinManager.idFromName(entry.getKey());
	...
	{
		Identifier id = Identifier.fromNamespaceAndPath(MocapMod.MOD_ID, "dummy_skin/waiting_for_skin");
