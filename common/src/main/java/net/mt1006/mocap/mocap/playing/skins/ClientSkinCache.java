...
{
	public static final ResourceLocation WAITING_FOR_SKIN = createDummySkin();
	private static final long MAX_CUMULATIVE_TEXTURE_SIZE = 384L * (1 << 20);
	private static final int MAX_ELEMENT_COUNT = 4096;
	private final Map<String, ResourceLocation> textureByName = new ConcurrentHashMap<>();
	private long sizeSum = 0L;
	private boolean playerWarned = false;

	public @Nullable ResourceLocation get(String name)
	{
	...
	public void clear()
	{
		...
		for (Map.Entry<String, ResourceLocation> entry : textureByName.entrySet())
		{
			ResourceLocation val = entry.getValue();
	...
	private static ResourceLocation createDummySkin()
	{
		return ResourceLocation.fromNamespaceAndPath(MocapMod.MOD_ID, "dummy_skin/waiting_for_skin");
	}
}
