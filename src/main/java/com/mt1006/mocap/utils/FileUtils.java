...
public class FileUtils
{
	...
	public static boolean saveRecording(CommandSourceStack commandSource, String name, ArrayList<Byte> recording)
	{
		...
		try
		{
			...
			{
				commandSource.sendFailure(new TranslatableComponent("mocap.commands.recording.save.file_already_exist"));
				commandSource.sendFailure(new TranslatableComponent("mocap.commands.recording.save.file_already_exist.tip"));
				return false;
			}
		...
		{
			commandSource.sendFailure(new TranslatableComponent("mocap.commands.recording.save.exception"));
			return false;
		}

		commandSource.sendSuccess(new TranslatableComponent("mocap.commands.recording.save.success"), false);
		return true;
	}
	...
	public static boolean removeRecording(CommandSourceStack commandSource, String name)
	{
		...
		{
			commandSource.sendFailure(new TranslatableComponent("mocap.commands.recording.remove.failed"));
			return false;
		}

		commandSource.sendSuccess(new TranslatableComponent("mocap.commands.recording.remove.success"), false);
		return true;
	}
	...
	public static boolean addScene(CommandSourceStack commandSource, String name)
	{
		...
		try
		{
			if (sceneFile.exists())
			{
				commandSource.sendFailure(new TranslatableComponent("mocap.commands.scenes.add.error"));
				commandSource.sendFailure(new TranslatableComponent("mocap.commands.scenes.add.error.file_already_exists"));
				return false;
			}
		...
		commandSource.sendSuccess(new TranslatableComponent("mocap.commands.scenes.add.success"), false);
		return true;
	}
	...
	public static boolean removeScene(CommandSourceStack commandSource, String name)
	{
		...
		if (!sceneFile.delete())
		{
			commandSource.sendFailure(new TranslatableComponent("mocap.commands.scenes.remove.failed"));
			return false;
		}

		commandSource.sendSuccess(new TranslatableComponent("mocap.commands.scenes.remove.success"), false);
		return true;
	}
	...
	public static boolean addToScene(CommandSourceStack commandSource, String name, String lineToAdd)
	{
		...
		try
		{
			if (!sceneFile.exists())
			{
				commandSource.sendFailure(new TranslatableComponent("mocap.commands.scenes.add_to.error"));
				commandSource.sendFailure(new TranslatableComponent("mocap.commands.scenes.add_to.error.file_not_exists"));
				return false;
			}
		...
		commandSource.sendSuccess(new TranslatableComponent("mocap.commands.scenes.add_to.success"), false);
		return true;
	}
	...
	public static boolean removeFromScene(CommandSourceStack commandSource, String name, int pos)
	{
		...
		try
		{
			if (!sceneFile.exists())
			{
				commandSource.sendFailure(new TranslatableComponent("mocap.commands.scenes.remove_from.error"));
				commandSource.sendFailure(new TranslatableComponent("mocap.commands.scenes.remove_from.error.file_not_exists"));
				return false;
			}
			...
			{
				commandSource.sendFailure(new TranslatableComponent("mocap.commands.scenes.remove_from.error"));
				commandSource.sendFailure(new TranslatableComponent("mocap.commands.scenes.remove_from.error.wrong_element_pos"));
				commandSource.sendFailure(new TranslatableComponent("mocap.commands.scenes.remove_from.error.wrong_element_pos.tip"));
				return false;
			}
		...
		commandSource.sendSuccess(new TranslatableComponent("mocap.commands.scenes.remove_from.success"), false);
		return true;
	}
	...
	public static boolean initDirectories(CommandSourceStack commandSource)
	{
		...
		{
			commandSource.sendFailure(new TranslatableComponent("mocap.commands.error.failed_to_init_directories"));
			return false;
		}
	...
	private static boolean checkIfProperName(String name, @Nullable CommandSourceStack commandSource)
	{
		...
		if (name.charAt(0) == '.')
		{
			commandSource.sendFailure(new TranslatableComponent("mocap.commands.error.improper_name"));
			commandSource.sendFailure(new TranslatableComponent("mocap.commands.error.improper_name.dot_first"));
			return false;
		}
		...
		for (char c : name.toCharArray())
		{
					...
					c != '_' && c != '-' && c != '.')
			{
				...
				{
					commandSource.sendFailure(new TranslatableComponent("mocap.commands.error.improper_name"));
					commandSource.sendFailure(new TranslatableComponent("mocap.commands.error.improper_name.character"));
				}
				return false;
			}
		}
