...
public class EntityData<T>
{
	...
	*/
	...
	public EntityData(Entity entity, DataIndex<T> index, T value)
	{
		...
		EntityDataAccessor<T> dataAccessor = new EntityDataAccessor<>(index.index, index.serializer);
		dataManager.define(dataAccessor, value);
	}

	public ClientboundSetEntityDataPacket getPacket()
	{
		return new ClientboundSetEntityDataPacket(entity.getId(), dataManager, true);
	}
