...
public class SceneFiles
{
	...
	public static boolean modify(CommandContext<CommandSourceStack> ctx, String name, int pos)
	{
	...
	private static @Nullable SceneData.Subscene modifySubscene(CommandContext<CommandSourceStack> rootCtx, SceneData.Subscene oldSubscene)
	{
		CommandSourceStack commandSource = rootCtx.getSource();
		SceneData.Subscene subscene = oldSubscene.copy();

		CommandContext<CommandSourceStack> ctx = CommandUtils.getFinalCommandContext(rootCtx);
		if (ctx == null)
		{
		...
		try
		{
			switch (propertyName)
			{
				...
				case "playerAsEntity":
					...
					{
						subscene.playerAsEntityID = ResourceArgument.getEntityType(ctx, "entity").key().location().toString();
						return subscene;
					}
	...
	public static boolean elementInfo(CommandSourceStack commandSource, String name, int pos)
	{
		File sceneFile = Files.getSceneFile(commandSource, name);
		if (sceneFile == null) { return false; }
		...
		switch (subscene.playerData.skinSource)
		{
			...
			case FROM_MINESKIN:
				...
				Component urlComponent = Utils.getURLComponent(subscene.playerData.skinPath, "  (§n%s§r)", subscene.playerData.skinPath);
	...
	public static boolean listElements(CommandSourceStack commandSource, String name)
	{
		SceneData sceneData = new SceneData();
		if(!sceneData.load(commandSource, name)) { return false; }
	...
	public static @Nullable List<String> list(MinecraftServer server, @Nullable CommandSourceStack commandSource)
	{
