...
public class Recording
{
	...
	public static int start(CommandSourceStack commandSource, ServerPlayer serverPlayer)
	{
		switch (state)
		{
			case NOT_RECORDING:
				...
				commandSource.sendSuccess(new TranslatableComponent("mocap.commands.recording.start.waiting_for_action"), false);
			...
			case WAITING_FOR_ACTION:
				...
				commandSource.sendSuccess(new TranslatableComponent("mocap.commands.recording.start.recording_started"), false);
				break;
			...
			case WAITING_FOR_DECISION:
				commandSource.sendFailure(new TranslatableComponent("mocap.commands.recording.stop.waiting_for_decision"));
				return 0;
		}
	...
	public static int stop(CommandContext<CommandSourceStack> ctx)
	{
		...
		{
			case NOT_RECORDING:
				commandSource.sendFailure(new TranslatableComponent("mocap.commands.recording.stop.server_not_recording"));
				return 0;

			case WAITING_FOR_ACTION:
				commandSource.sendSuccess(new TranslatableComponent("mocap.commands.recording.stop.stop_waiting_for_action"), false);
				state = State.NOT_RECORDING;
				break;

			case RECORDING:
				commandSource.sendSuccess(new TranslatableComponent("mocap.commands.recording.stop.waiting_for_decision"), false);
				state = State.WAITING_FOR_DECISION;
				break;

			case WAITING_FOR_DECISION:
				recording.clear();
				commandSource.sendSuccess(new TranslatableComponent("mocap.commands.recording.stop.recording_discarded"), false);
				state = State.NOT_RECORDING;
				break;
		}
	...
	public static int list(CommandContext<CommandSourceStack> ctx)
	{
		...
		if (recordingsList == null)
		{
			...
			recordingsListStr.append(new TranslatableComponent("mocap.commands.playing.list.error").getString());
		}
		...
		{
			recordingsListStr.append(" ");
			recordingsListStr.append(new TranslatableComponent("mocap.commands.playing.list.empty").getString());
		}

		commandSource.sendSuccess(new TranslatableComponent("mocap.commands.playing.list.recordings",
	...
	public static int state(CommandContext<CommandSourceStack> ctx)
	{
		...
		{
			case NOT_RECORDING:
				commandSource.sendSuccess(new TranslatableComponent("mocap.commands.recording.state.not_recording"), false);
				break;

			case WAITING_FOR_ACTION:
				commandSource.sendSuccess(new TranslatableComponent("mocap.commands.recording.state.waiting_for_action"), false);
				commandSource.sendSuccess(new TranslatableComponent("mocap.commands.recording.state.player_name", serverPlayer.getName()), false);
				break;

			case RECORDING:
				commandSource.sendSuccess(new TranslatableComponent("mocap.commands.recording.state.recording"), false);
				commandSource.sendSuccess(new TranslatableComponent("mocap.commands.recording.state.player_name", serverPlayer.getName()), false);
				break;

			case WAITING_FOR_DECISION:
				commandSource.sendSuccess(new TranslatableComponent("mocap.commands.recording.state.waiting_for_decision"), false);
				break;
		}
		return 1;
	}
