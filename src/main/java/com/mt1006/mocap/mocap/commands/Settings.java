...
public class Settings
{
	...
	public static int info(CommandContext<CommandSourceStack> ctx)
	{
		...
		if (setting == null)
		{
			commandSource.sendFailure(new TranslatableComponent("mocap.commands.settings.error"));
			return 0;
		}
		...
		commandSource.sendSuccess(new TranslatableComponent("mocap.commands.settings.info.val", setting.val.toString()), false);
		commandSource.sendSuccess(new TranslatableComponent("mocap.commands.settings.info.def_val", setting.defVal.toString()), false);
	...
	public static int set(CommandContext<CommandSourceStack> ctx)
	{
		...
		{
			commandSource.sendFailure(new TranslatableComponent("mocap.commands.settings.set.error"));
		}

		commandSource.sendSuccess(new TranslatableComponent("mocap.commands.settings.set", oldValue, setting.val.toString()), false);
		save(commandSource);
		return 1;
	}
}
