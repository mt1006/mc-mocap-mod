...
public class Playing
{
	...
	public static int start(CommandSourceStack commandSource, String name)
	{
		...
		commandSource.sendSuccess(Component.translatable("mocap.commands.playing.start.success"), false);
		return 1;
	}
	...
	public static int stop(CommandSourceStack commandSource, int id)
	{
		for (PlayedScene scene : playedScenes)
		{
			...
			{
				scene.stop();
				commandSource.sendSuccess(Component.translatable("mocap.commands.playing.stop.success"), false);
				return 1;
			}
		}

		commandSource.sendFailure(Component.translatable("mocap.commands.playing.stop.unable_to_find_scene"));
		commandSource.sendFailure(Component.translatable("mocap.commands.playing.stop.unable_to_find_scene.tip"));
		return 0;
	}
	...
	public static int stopAll(CommandContext<CommandSourceStack> ctx)
	{
		...
		ctx.getSource().sendSuccess(Component.translatable("mocap.commands.playing.stop_all.success"), false);
		return 1;
	}
	...
	public static int list(CommandContext<CommandSourceStack> ctx)
	{
		...
		commandSource.sendSuccess(Component.translatable("mocap.commands.playing.list.playing"), false);

		for (PlayedScene scene : playedScenes)
		{
			commandSource.sendSuccess(Component.literal(String.format("[%d] %s", scene.getID(), scene.getName())), false);
		}
