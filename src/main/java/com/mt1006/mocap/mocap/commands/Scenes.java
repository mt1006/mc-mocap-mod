...
public class Scenes
{
	public static int listElements(CommandSourceStack commandSource, String name)
	{
		...
		commandSource.sendSuccess(new TranslatableComponent("mocap.commands.scenes.list_elements.list"), false);

		int i = 1;
		for (SceneInfo.Subscene element : sceneInfo.subscenes)
		{
			commandSource.sendSuccess(new TextComponent(String.format("[%d] %s <%f> [%f; %f; %f] (%s)",
					i, element.getName(), element.startDelay, element.startPos[0], element.startPos[1], element.startPos[2],
					element.playerName)), false);
			i++;
		}

		commandSource.sendSuccess(new TextComponent("[id] name <startDelay> [x; y; z] (playerName)"), false);
		return 1;
	}
	...
	public static int list(CommandContext<CommandSourceStack> ctx)
	{
		...
		if (scenesList == null)
		{
			...
			scenesListStr.append(new TranslatableComponent("mocap.commands.playing.list.error").getString());
		}
		...
		{
			scenesListStr.append(" ");
			scenesListStr.append(new TranslatableComponent("mocap.commands.playing.list.empty").getString());
		}

		commandSource.sendSuccess(new TranslatableComponent("mocap.commands.playing.list.scenes",
