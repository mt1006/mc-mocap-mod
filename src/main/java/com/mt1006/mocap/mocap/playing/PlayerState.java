...
public class PlayerState
{
	...
	public PlayerState(PlayerEntity player)
	{
		...
		rotation[0] = player.xRot;
		rotation[1] = player.yRot;
		...
		if (player.getUsedItemHand() == Hand.OFF_HAND) { entityFlags |= 0x02; }
	...
	public void saveDifference(ArrayList<Byte> recording, @Nullable PlayerState previousState)
	{
		...
		if (swinging && (previousState == null || !previousState.swinging || previousState.swingingTime > swingingTime))
		{
			...
			RecordingUtils.addBoolean(recording, (swingingHand == Hand.OFF_HAND));
		}

		RecordingUtils.addByte(recording, NEXT_TICK);
	}

	public static int readState(PlayerList packetTargets, PlayerEntity player, RecordingUtils.RecordingReader reader)
	{
		if ((int)reader.recording.length == reader.offset) { return RET_END; }
		...
		try
		{
			switch (reader.readByte())
			{
				...
				case PLAYER_MOVEMENT:
					...
					packetTargets.broadcastAll(new SEntityTeleportPacket(player));
					return RET_OK;
				...
				case CHANGE_POSE:
					...
					EntityDataManager poseDataManager = new EntityDataManager(player);
					DataParameter<Pose> poseParameter = new DataParameter<>(6, DataSerializers.POSE);
					poseDataManager.define(poseParameter, newPose);

					packetTargets.broadcastAll(new SEntityMetadataPacket(player.getId(), poseDataManager, true));
					return RET_OK;

				case CHANGE_ITEM:
					List<Pair<EquipmentSlotType, ItemStack>> list = new ArrayList<>();

					if (reader.readBoolean()) { list.add(new Pair<>(EquipmentSlotType.MAINHAND, new ItemStack(Item.byId(reader.readInt())))); }
					if (reader.readBoolean()) { list.add(new Pair<>(EquipmentSlotType.OFFHAND, new ItemStack(Item.byId(reader.readInt())))); }
					if (reader.readBoolean()) { list.add(new Pair<>(EquipmentSlotType.FEET, new ItemStack(Item.byId(reader.readInt())))); }
					if (reader.readBoolean()) { list.add(new Pair<>(EquipmentSlotType.LEGS, new ItemStack(Item.byId(reader.readInt())))); }
					if (reader.readBoolean()) { list.add(new Pair<>(EquipmentSlotType.CHEST, new ItemStack(Item.byId(reader.readInt())))); }
					if (reader.readBoolean()) { list.add(new Pair<>(EquipmentSlotType.HEAD, new ItemStack(Item.byId(reader.readInt())))); }

					packetTargets.broadcastAll(new SEntityEquipmentPacket(player.getId(), list));
					return RET_OK;
				...
				case SET_ENTITY_FLAGS:
					...
					//https://wiki.vg/index.php?title=Entity_metadata&oldid=15886#Entity
					EntityDataManager entityFlagsDataManager = new EntityDataManager(player);
					DataParameter<Byte> entityFlagsParameter = new DataParameter<>(0, DataSerializers.BYTE);
					entityFlagsDataManager.define(entityFlagsParameter, newEntityFlags);

					packetTargets.broadcastAll(new SEntityMetadataPacket(player.getId(), entityFlagsDataManager, true));
					return RET_OK;
				...
				case SET_LIVING_ENTITY_FLAGS:
					...
					EntityDataManager livingEntityFlagsDataManager = new EntityDataManager(player);
					DataParameter<Byte> livingEntityFlagsParameter = new DataParameter<>(7, DataSerializers.BYTE);
					livingEntityFlagsDataManager.define(livingEntityFlagsParameter, newLivingEntityFlags);

					packetTargets.broadcastAll(new SEntityMetadataPacket(player.getId(), livingEntityFlagsDataManager, true));
					return RET_OK;
				...
				case SET_MAIN_HAND:
					...
					EntityDataManager setMainHandDataManager = new EntityDataManager(player);
					DataParameter<Byte> setMainHandParameter = new DataParameter<>(17, DataSerializers.BYTE);
					setMainHandDataManager.define(setMainHandParameter, newMainHand);

					packetTargets.broadcastAll(new SEntityMetadataPacket(player.getId(), setMainHandDataManager, true));
					return RET_OK;
				...
				case SWING:
					...
					packetTargets.broadcastAll(new SAnimateHandPacket(player, eventID));
					return RET_OK;
			}
		}
	...
	public static boolean readHeader(PlayerEntity player, RecordingUtils.RecordingReader reader, Vector3d posOffset)
	{
		try
		{
