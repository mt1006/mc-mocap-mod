...
public class MocapCommand
{
	...
	private static int recordingStart(CommandContext<CommandSource> ctx)
	{
		...
		try
		{
			...
			{
				ctx.getSource().sendFailure(new TranslationTextComponent("mocap.commands.recording.start.player_not_found"));
				return 0;
			}
	...
	{
		ctx.getSource().sendSuccess(new StringTextComponent(MocapMod.getFullName()), false);
		ctx.getSource().sendSuccess(new StringTextComponent("Author: mt1006 (mt1006x)"), false);
		return 1;
	}

	private static int help(CommandContext<CommandSource> ctx)
	{
		ctx.getSource().sendSuccess(new TranslationTextComponent("mocap.commands.help", MocapMod.getName()), false);
		return 1;
	}
}
