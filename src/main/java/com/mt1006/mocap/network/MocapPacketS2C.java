...
public class MocapPacketS2C
{
	...
	public static void sendOnLogin(ServerPlayerEntity serverPlayer)
	{
		respond(serverPlayer, ON_LOGIN, null);
	}

	public static void sendNocolPlayerAdd(ServerPlayerEntity serverPlayer, UUID player)
	{
		send(serverPlayer, NOCOL_PLAYER_ADD, player);
	}

	public static void sendNocolPlayerRemove(ServerPlayerEntity serverPlayer, UUID player)
	{
		send(serverPlayer, NOCOL_PLAYER_REMOVE, player);
	}

	public static void sendInputSuggestionsAdd(ServerPlayerEntity serverPlayer, Collection<String> strings)
	{
		send(serverPlayer, INPUT_SUGGESTIONS_ADD, strings);
	}

	public static void sendInputSuggestionsAddOnLogin(ServerPlayerEntity serverPlayer, Collection<String> strings)
	{
		respond(serverPlayer, INPUT_SUGGESTIONS_ADD, strings);
	}

	public static void sendInputSuggestionsRemove(ServerPlayerEntity serverPlayer, Collection<String> strings)
	{
		send(serverPlayer, INPUT_SUGGESTIONS_REMOVE, strings);
	}

	public static void sendCustomSkinData(ServerPlayerEntity serverPlayer, String name, byte[] byteArray)
	{
		send(serverPlayer, CUSTOM_SKIN_DATA, new Pair<>(name, byteArray));
	}

	private static void send(ServerPlayerEntity serverPlayer, int op, Object object)
	{
	...
	private static void respond(ServerPlayerEntity serverPlayer, int op, Object object)
	{
